...
// @access  Public
router.delete("/deleteAssignments/:id",authTeacher, async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (assignment) {
      let teacher = await Teacher.findById(req.user.user.id);
      let result = teacher.sectionAccess.find((el)=>{return el.sectionName==assignment.sectionName.toString();});
      if(!result){
        return res.status(404).json({ errors: "Unauthorized Access" });
      }
...
router.get("/readAssignments",authTeacher, async (req, res) => {
  try {
    let qdata = req.query;
...
router.get("/subjectsAssignmentForStudent/:studentId",authTeacher, async (req, res) => {
  try {
    ...
    console.log(student);
    console.log(teacher);
    let result = teacher.sectionAccess.find((el)=>{return el.sectionName==student.sectionName.toString()});
    if(!result){
      return res.status(404).json({ errors: "Unauthorized Access" });
    }
...
router.delete("/deleteHomeworks/:id", async (req, res) => {
  ...
  try {
    ...
    if (homework) {
      let teacher = await Teacher.findById(req.user.user.id);
      let result = teacher.sectionAccess.find((el)=>{return el.sectionName==homework.sectionName.toString();});
      if(!result){
        return res.status(404).json({ errors: "Unauthorized Access" });
      }
      homework.deleteOne();
...
router.get("/readHomeworks",authTeacher, async (req, res) => {
  try {
...
router.get("/subjectsHomeWorkForStudent/:studentId",authTeacher, async (req, res) => {
  try {
    // Check if the student exist
    const { studentId } = req.params;
